/* ═══════════════════════════════════════════════════
   PiCar-X Control — app.js
   Dashboard layout: Drive always visible,
   Camera/Voice/AI/Settings in tabs.
...
function cacheDom() {
  // Tab navigation (Drive is always visible, not in tabs)
  el.tabs      = $$(".tab");
...
function setSpeechStatus(msg, tone = "neutral") {
  if (el.speechStatus) { el.speechStatus.textContent = msg; el.speechStatus.dataset.tone = tone; }
}

function setSettingsStatus(msg, tone = "neutral") {
  if (el.settingsSaveStatus) { el.settingsSaveStatus.textContent = msg; el.settingsSaveStatus.dataset.tone = tone; }
}
...
function updateMessageCount() {
  el.messageCountLabel.textContent = `${el.messages.childElementCount}`;
}
...
function updateMicBadge() {
  ...
  if (app.captureActive) {
    badge.textContent = "Listening"; badge.dataset.state = "listening";
  } else if (app.openMic && app.ws?.readyState === WebSocket.OPEN) {
    badge.textContent = "Ready"; badge.dataset.state = "on";
  } else {
...
function logMessage(role, text) {
  ...
  const pill = document.createElement("span"); pill.className = "role-pill"; pill.textContent = titleCase(role);
  const time = document.createElement("span"); time.className = "message-time"; time.textContent = formatTimestamp(new Date().toISOString());
  const body = document.createElement("div"); body.className = "message-body"; body.textContent = text;
  head.append(pill, time); row.append(head, body);
  el.messages.prepend(row);
  while (el.messages.childElementCount > CONFIG.maxMessages) el.messages.lastElementChild?.remove();
  updateMessageCount();
}
...
async function refreshState() {
  ...
  app.refreshPromise = (async () => {
    ...
    render(session); renderHealth(health);
  })();
  try { await app.refreshPromise; }
  finally {
    app.refreshPromise = null;
    if (app.refreshQueued) { app.refreshQueued = false; queueMicrotask(() => refreshState().catch(() => null)); }
  }
}
...
function bytesFromBase64(enc) {
  const bin = atob(enc); const b = new Uint8Array(bin.length);
...
function getPlaybackGain() { return app.volume / 100; }
...
async function playRelayChunk(audioBase64, sampleRate) {
  ...
  const src = app.audioContext.createBufferSource(); src.buffer = buffer;
  const gain = app.audioContext.createGain(); gain.gain.value = getPlaybackGain();
  src.connect(gain).connect(app.audioContext.destination);
  const at = Math.max(app.audioContext.currentTime + 0.01, app.playbackCursor);
  src.start(at); app.playbackCursor = at + buffer.duration;
}
...
async function playAssistantAudio(audioBase64) {
  ...
  const src = app.audioContext.createBufferSource(); src.buffer = buffer;
  const gain = app.audioContext.createGain(); gain.gain.value = getPlaybackGain();
  src.connect(gain).connect(app.audioContext.destination); src.start();
}

/* ══════════════════════════════════════════
   VOICE — WebSocket + Capture
   ══════════════════════════════════════════ */
...
function closeVoiceSocket(reason = "Client closing") {
  ...
  app.ws = null; app.socketPromise = null;
}
...
function handleVoiceSocketMessage(payload) {
  ...
  if (payload.type === "error")           { app.awaitingReply = false; setSpeechStatus(payload.message, "danger"); }
}
...
async function openVoiceSocket() {
  ...
  const url = `${proto}://${location.host}${ENDPOINTS.voiceSocket}`;
  app.socketPromise = new Promise((resolve, reject) => {
    const sock = new WebSocket(url); app.ws = sock;
    sock.addEventListener("message", e => handleVoiceSocketMessage(JSON.parse(e.data)));
    sock.addEventListener("open", () => resolve(sock), { once: true });
    sock.addEventListener("error", () => reject(new Error("Unable to open voice link.")), { once: true });
    sock.addEventListener("close", () => {
      app.ws = null; app.socketPromise = null; app.captureActive = false; app.awaitingReply = false;
      setButtonActive(el.pushToTalkBtn, false); updateMicBadge();
      setSpeechStatus("Voice link closed. Will reconnect on next use.", "neutral");
    });
  });
  try { return await app.socketPromise; } finally { app.socketPromise = null; }
}
...
async function ensureCapturePipeline() {
  ...
  app.monitorNode = app.audioContext.createGain(); app.monitorNode.gain.value = 0;
  app.workletNode.port.onmessage = (ev) => {
    if (!app.captureActive || !app.ws || app.ws.readyState !== WebSocket.OPEN) return;
    app.ws.send(JSON.stringify({ type: "pcm_chunk", audio: base64FromArrayBuffer(ev.data) }));
  };
...
function configureSpeechRecognition() {
  ...
  app.recognition.lang = "en-US"; app.recognition.continuous = true; app.recognition.interimResults = true;
  app.recognition.onresult = (ev) => {
    const chunks = [];
    for (let i = ev.resultIndex; i < ev.results.length; i++) chunks.push(ev.results[i][0].transcript);
...
async function startTalking() {
  ...
  await openVoiceSocket(); await ensureCapturePipeline(); configureSpeechRecognition();
  app.transcript = ""; app.awaitingReply = false; app.captureActive = true;
  setButtonActive(el.pushToTalkBtn, true); updateMicBadge();
  if (app.session?.voice_mode === "ai_reply" && app.recognition) { try { app.recognition.start(); } catch (_) {} }
  setSpeechStatus("Listening...", "cool");
}

async function stopTalking() {
  if (!app.captureActive) return;
  app.captureActive = false; setButtonActive(el.pushToTalkBtn, false); updateMicBadge();
  if (app.recognition) { try { app.recognition.stop(); } catch (_) {} }
...
async function stopDriveLoop() {
  ...
  clearDriveLoop();
  const s = await api(ENDPOINTS.driveStop, { method: "POST" }).catch(() => null);
  if (s) render(s);
}
...
async function updateCamera(pan, tilt) { await applySessionAction(ENDPOINTS.camera, { pan, tilt }); }

async function moveCameraBy(dP, dT) {
  if (!app.session) return;
  const pan = Number(app.session.camera.pan) + dP;
  const tilt = Number(app.session.camera.tilt) + dT;
  el.panSlider.value = String(pan); el.tiltSlider.value = String(tilt);
  syncRangeReadouts(); await updateCamera(pan, tilt);
}
...
async function submitVisionQuestion(q) {
  if (!q) return;
  el.visionAnswer.textContent = "Thinking...";
  try { const r = await api(ENDPOINTS.visionQuestion, { method: "POST", json: { question: q } }); el.visionAnswer.textContent = r.answer; }
  catch (e) { el.visionAnswer.textContent = e.message; }
}
...
async function saveSettingsPatch(patch) {
  ...
  app.settingsDirty = false; syncSettingsForm(session.settings, true);
  setSettingsStatus("Settings saved.", "ok"); return session;
}
...
function bindMomentaryPointerControl(node, { start, stop }) {
  let pid = null;
  const release = () => { if (pid === null) return; pid = null; Promise.resolve(stop()).catch(e => setSpeechStatus(e.message, "danger")); };
  node.addEventListener("pointerdown", ev => {
    if (pid !== null || (ev.pointerType === "mouse" && ev.button !== 0)) return;
    pid = ev.pointerId;
    try { node.setPointerCapture(ev.pointerId); } catch (_) {}
    ev.preventDefault();
    Promise.resolve(start(ev)).catch(e => { pid = null; setSpeechStatus(e.message, "danger"); });
  });
...
function bindDriveButton(btn) {
  const ss = Number(btn.dataset.speedSign), st = Number(btn.dataset.steering);
  if (ss === 0 && st === 0) { btn.addEventListener("click", () => stopDriveLoop().catch(() => null)); return; }
  bindMomentaryPointerControl(btn, {
    start: () => startDriveLoop(buildDriveCommand(ss, st, "browser"), btn),
    stop: () => stopDriveLoop(),
  });
}

function bindKeyboard() {
  const km = { w: () => buildDriveCommand(1, 0, "keyboard"), a: () => buildDriveCommand(1, -25, "keyboard"), d: () => buildDriveCommand(1, 25, "keyboard"), s: () => buildDriveCommand(-1, 0, "keyboard") };
  window.addEventListener("keydown", ev => {
    const k = ev.key.toLowerCase();
    if (!km[k] || app.activeKey === k || ev.repeat) return;
    if (["INPUT","SELECT","TEXTAREA"].includes(ev.target.tagName)) return;
    app.activeKey = k; ev.preventDefault(); startDriveLoop(km[k]()).catch(() => null);
  });
  window.addEventListener("keyup", ev => { if (ev.key.toLowerCase() !== app.activeKey) return; app.activeKey = null; stopDriveLoop().catch(() => null); });
  window.addEventListener("blur", () => { app.activeKey = null; stopDriveLoop().catch(() => null); });
}
...
function stopDriveOnUnload() {
  const p = new Blob(["{}"], { type: "application/json" });
  if (navigator.sendBeacon) { navigator.sendBeacon(ENDPOINTS.driveStop, p); return; }
  fetch(ENDPOINTS.driveStop, { method: "POST", body: "{}", headers: { "Content-Type": "application/json" }, keepalive: true }).catch(() => null);
}

function registerGlobalCleanup() {
  window.addEventListener("beforeunload", () => { stopDriveOnUnload(); teardownCapturePipeline(); closeVoiceSocket("Browser unloading"); });
}
...
async function init() {
  cacheDom();
  showPanel("camera");     // default secondary tab
  syncRangeReadouts();
  updateMessageCount();
  ...
  el.estopFab.addEventListener("click", async () => {
    ...
    else { clearDriveLoop(); await applySessionAction(ENDPOINTS.emergencyStop); }
  });
  el.estopResetBtn.addEventListener("click", () => applySessionAction(ENDPOINTS.emergencyReset));

  // ── Drive stop
  el.stopBtn.addEventListener("click", async () => { clearDriveLoop(); await applySessionAction(ENDPOINTS.driveStop); });
  ...
  el.volumeSlider.addEventListener("input", () => { app.volume = Number(el.volumeSlider.value); syncRangeReadouts(); });
  ...
  el.camCenter.addEventListener("click", () => { el.panSlider.value = "0"; el.tiltSlider.value = "0"; syncRangeReadouts(); updateCamera(0, 0); });
  el.centerCameraBtn.addEventListener("click", () => { el.panSlider.value = "0"; el.tiltSlider.value = "0"; syncRangeReadouts(); updateCamera(0, 0); });
  ...
  el.greetingEnabledToggle.addEventListener("change", () => saveSettingsPatch({ greeting_enabled: el.greetingEnabledToggle.checked }));
  el.greetingModeSelect.addEventListener("change", () => saveSettingsPatch({ greeting_mode: el.greetingModeSelect.value }));

  // ── Vision Q&A
  el.visionForm.addEventListener("submit", ev => { ev.preventDefault(); submitVisionQuestion(el.visionQuestion.value.trim()); });
  el.promptChips.forEach(b => b.addEventListener("click", () => { el.visionQuestion.value = b.dataset.prompt ?? ""; submitVisionQuestion(b.dataset.prompt); }));

  // ── Settings form
  el.settingsForm.addEventListener("submit", async ev => {
    ev.preventDefault();
    const p = settingsPayloadFromForm();
    if (!p.greeting_text) { setSettingsStatus("Greeting text cannot be blank.", "danger"); return; }
    try { const s = await applySessionAction(ENDPOINTS.settings, p); app.settingsDirty = false; syncSettingsForm(s.settings, true); setSettingsStatus("Settings saved.", "ok"); }
    catch (e) { setSettingsStatus(e.message, "danger"); }
  });
  el.settingsEstopBtn.addEventListener("click", async () => { clearDriveLoop(); await applySessionAction(ENDPOINTS.emergencyStop); });
