...
class RobotRuntime:
    ...
    def current_session(self) -> RobotSession:
        ...
        session.drive = DriveState(
            speed=hardware.drive_speed,
            steering=hardware.steering,
            last_command_at=session.drive.last_command_at,
        )
        session.camera = CameraState(
            pan=hardware.pan,
            tilt=hardware.tilt,
            last_command_at=session.camera.last_command_at,
        )
    ...
    def set_voice_mode(self, mode: VoiceMode) -> RobotSession:
        ...
        self.publish_browser_event({"type": "state", "state": self.current_session().model_dump(mode="json")})
        return session
    ...
    def record_error(self, message: str | None) -> RobotSession:
        return self.store.update(lambda state: setattr(state, "last_error", message))
