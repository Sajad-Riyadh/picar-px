...
class RobotRuntime:
    ...
    def set_voice_mode(self, mode: VoiceMode) -> RobotSession:
        session = self.store.update(lambda state: setattr(state, "voice_mode", mode))
        return self._publish_state(session)

    def set_audio_target(self, target: AudioTarget) -> RobotSession:
        session = self.store.update(lambda state: setattr(state, "audio_target", target))
        return self._publish_state(session)
    ...
    def update_settings(self, request: SettingsUpdateRequest) -> RobotSession:
        ...
        session = self.store.update(mutate)
        ...
        if not settings.autonomous_mode_enabled:
            ...
            session = self.store.update(
                lambda state: setattr(
                    ...
                    "Autonomous mode disabled from settings.",
                )
            )
        return self._publish_state(session)
    ...
    def record_error(self, message: str | None, *, publish: bool = True) -> RobotSession:
        ...
        if session.last_error == message:
            return self.current_session() if publish else session
        session = self.store.update(lambda state: setattr(state, "last_error", message))
        return self._publish_state(session) if publish else session
    ...
    async def handle_ai_turn(self, transcript: str) -> str:
        snapshot = self.vision.get_snapshot()
        session = self.store.load()
        reply, native_wav = await self.ai.generate_reply(transcript, snapshot.summary)
        wav_bytes = native_wav or self.ai.synthesize(reply)
    ...
    def register_browser_client(self, websocket: object) -> None:
        ...
        session = self.store.update(lambda state: setattr(state, "browser_connected", True))
    ...
    def unregister_browser_client(self, websocket: object) -> None:
        ...
        session = self.store.update(lambda state: setattr(state, "browser_connected", connected))
        self._publish_state(session)
    ...
    def publish_browser_event(self, payload: dict) -> None:
        ...
        for websocket in clients:
            self._schedule_browser_send(websocket, message, running_loop)

    def record_behavior_action(self, action: str) -> RobotSession:
        session = self.store.load()
        if session.last_behavior_action == action:
            return self.current_session()
        session = self.store.update(lambda state: setattr(state, "last_behavior_action", action))
        return self._publish_state(session)

    def record_autonomy_action(self, action: str) -> RobotSession:
        session = self.store.load()
        if session.last_autonomy_action == action:
            return self.current_session()
        session = self.store.update(lambda state: setattr(state, "last_autonomy_action", action))
        return self._publish_state(session)
    ...
    def _apply_drive_command(
        ...
        *,
        ...
        if publish_state:
            session = self._publish_state(session)
        if fast:
    ...
    def _publish_state(self, session: RobotSession | None = None) -> RobotSession:
        session = self._prepare_session(session or self.store.load())
        self.publish_browser_event({"type": "state", "state": session.model_dump(mode="json")})
        return session

    def _prepare_session(self, session: RobotSession) -> RobotSession:
    ...
    def _schedule_browser_send(
        ...
        try:
            if running_loop is self._loop:
                task = self._loop.create_task(websocket.send_text(message))
                task.add_done_callback(
                    lambda future, client=websocket: self._handle_browser_send_completion(
                        client,
                        future,
                    )
                )
                return
            future = asyncio.run_coroutine_threadsafe(websocket.send_text(message), self._loop)
            future.add_done_callback(
                lambda pending, client=websocket: self._handle_browser_send_completion(
                    client,
                    pending,
                )
            )
        except Exception:
            logger.exception("Failed to schedule browser event delivery; dropping websocket client.")
    ...
    def _drop_browser_client(self, websocket: object) -> None:
        with self._ws_lock:
            self._browser_clients.discard(websocket)
